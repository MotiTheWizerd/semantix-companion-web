...
//!
//! A companion is a name (optional; it may sit unnamed) plus ONE private
//! memory. That memory is an agent on the organ's roster, addressed by
...
impl CompanionService {
    ...
    fn update(&self, input: UpdateCompanionInput) -> Result<Companion, AppError> {
        let UpdateCompanionInput { companion_id, name } = input;
        let current = self.require(companion_id.trim())?;
        let name = normalise_name(name)?;
        let timestamp = unix_timestamp_ms()?;
        self.repository
            .rename(&current.id, name.as_deref(), timestamp)?;
...
mod tests {
    ...
    use super::{
        normalise_name, Companion, CompanionChangedEvent, CompanionRepository, CompanionService,
        CreateCompanionInput, UpdateCompanionInput, MAX_NAME_LENGTH,
    };
    use crate::database;
    ...
    fn each_added_companion_gets_its_own_private_memory() {
        ...
        let second = service
            .create(CreateCompanionInput { name: None })
    ...
    fn create_input_accepts_the_camel_case_ipc_contract() {
        let input: CreateCompanionInput =
            serde_json::from_value(serde_json::json!({ "name": "Ragnar" }))
